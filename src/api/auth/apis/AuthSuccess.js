...
const AuthSuccess = () => {
  const [accessTokenTransfer, setAccessTokenTransfer] = useState();
  ...
  //사용자 인증 성공 시 사용자 토큰발급. scope = transfer
  useEffect(() => {
    ...
    //axios.post("http://localhost:8080/api/v1/token/getToken", null, datas). 이렇게 하면 왜 contorller에서는 request.getParam이 null로 찍힘..? -> requestParam으로 받아서?

    axios
      .post("http://localhost:8080/api/v1/token/getToken", null, {
        params: {
          code: query,
          client_id: "9815d321-e0e1-462c-ad06-9cfa5d46c53c",
          client_secret: "39831a41-c3d7-47f2-b72f-bc82c4ca5e53",
      ...
      .catch((res) => {
        console.log("token API fail...", res);
      });
